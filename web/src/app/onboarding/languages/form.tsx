...
import { Alert, Button, ErrorBox, Field, Select, useSubmit } from "@/components/ui";
...
export function LanguagesForm({ wsId, settings, canEdit }: { wsId: string; settings: Record<string, unknown>; canEdit: boolean }) {
  ...
  const sel = (k: string, label: string, hint: string) => (
    <Field label={label} hint={hint} error={fieldError(error, k)}>
      <Select value={String(f[k])} onChange={(e) => { setSaved(false); setF({ ...f, [k]: e.target.value }); }} disabled={!canEdit}>
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); run(); }} className="space-y-4">
      ...
      <fieldset className="space-y-2 text-body-sm"><legend className="font-semibold text-primary">c) Tilladte samtalesprog</legend>
        ...
        {fieldError(error, "default_conversation_language") && <p className="text-label-sm text-error">Standardsproget skal være blandt de tilladte.</p>}
      </fieldset>
      {sel("report_language", "d) Rapportsprog", "Daglige resuméer på SMS/e-mail")}
      <p className="text-label-sm text-on-surface-variant">Stemmeprøve kræver stemmeadapteren, som tilkobles i receptionsmilepælen.</p>
