...
export function NewItemForm({ wsId, defaultKind = "service" }: { wsId: string; defaultKind?: string }) {
  const router = useRouter();
  const [kind, setKind] = useState(defaultKind);
  const [title, setTitle] = useState("");
  const [content, setContent] = useState(TEMPLATES[defaultKind]);
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); run(); }} className="space-y-3">
      ...
      <div className="grid gap-3">
        <Field label="Art"><Select value={kind} onChange={(e) => { setKind(e.target.value); setContent(TEMPLATES[e.target.value]); }}>{KINDS.map(([c, l]) => <option key={c} value={c}>{l}</option>)}</Select></Field>
...
function VersionActions({ wsId, v, canApprove }: { wsId: string; v: Version; canApprove: boolean }) {
  ...
  return (
    <span className="flex flex-wrap gap-2">
      ...
      {!canApprove && v.status === "in_review" && <span className="text-label-sm text-on-surface-variant">Afventer godkendelse af administrator</span>}
    </span>
  );
}
...
export function ReviewQueue({ wsId, queue, canApprove }: { wsId: string; queue: Version[]; canApprove: boolean }) {
  return (
    <ul className="divide-y divide-outline-variant/40 text-body-md">
      {queue.map((v) => (
        <li key={v.id} className="flex flex-wrap items-center justify-between gap-3 py-3">
          <span><strong>{v.title}</strong> <span className="text-on-surface-variant">v{v.version_no}</span> <Badge status={v.status} /></span>
...
export function KnowledgeList({ wsId, items, canDraft, canApprove }: { wsId: string; items: Item[]; canDraft: boolean; canApprove: boolean }) {
  ...
  if (items.length === 0) return <p className="text-body-sm text-on-surface-variant">Ingen vidensemner endnu. Opret det første nedenfor – mindst én ydelse og åbningstider er nødvendige.</p>;
  return (
    <ul className="divide-y divide-outline-variant/40 text-body-md">
      {items.map((i) => {
        ...
        return (
          <li key={i.id} className="space-y-2 py-3">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <span><span className="mr-2 rounded bg-surface-container-high px-2 py-0.5 text-label-sm text-on-surface-variant">{kindLabel(i.kind)}</span><strong>{v?.title}</strong>
                {i.approved_version && <> <Badge status="approved" /> <span className="text-label-sm text-on-surface-variant">v{i.approved_version.version_no}</span></>}
                {i.open_draft && <> <Badge status={i.open_draft.status} /> <span className="text-label-sm text-on-surface-variant">v{i.open_draft.version_no}</span></>}
              </span>
              <span className="flex flex-wrap gap-2">
                {i.open_draft && <VersionActions wsId={wsId} v={i.open_draft} canApprove={canApprove} />}
                {canDraft && editing !== i.id && <Button variant="tonal" className="text-label-md px-space-md py-2" onClick={() => { const src = i.open_draft ?? i.approved_version!; setEditing(i.id); setTitle(src.title); setText(JSON.stringify(src.content, null, 2)); }}>{i.open_draft ? "Redigér kladde" : "Ny kladde"}</Button>}
              </span>
            </div>
            {editing === i.id ? (
              <form onSubmit={(e) => { e.preventDefault(); run(); }} className="space-y-2 rounded-xl bg-surface-container-low p-space-md">
                <ErrorBox error={error} />
                ...
                <p className="text-label-sm text-on-surface-variant">{i.approved_version ? "Den godkendte version forbliver aktiv, indtil kladden godkendes." : ""}</p>
            ...
            ) : <pre className="overflow-x-auto rounded-lg bg-surface-container-low p-2 text-label-sm text-on-surface-variant">{JSON.stringify(v?.content)}</pre>}
          </li>
        );
      })}
    </ul>
